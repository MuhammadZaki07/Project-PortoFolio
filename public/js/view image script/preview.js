...
function previewProjectImage(event) {
    ...
    if (input.files && input.files[0]) {
        const reader = new FileReader();
        reader.onload = function (e) {
            const projectImagePreview = document.getElementById(
                "projectImagePreview"
            );
            projectImagePreview.src = e.target.result;
            const projectImagePreviewContainer = document.getElementById(
...
function previewCertificateImage(event) {
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            ...
            certificateImagePreview.classList.add(
                "max-w-xl",
                "h-auto",
                "rounded-lg",
                "object-cover"
            ...
            certificateImagePreviewContainer.appendChild(
